...
class CarlaSimulation(object):
    ...
    def synchronize_pedestrian(self, pedestrian_id, transform, direction, speed):
        """
        Updates pedestrian state.

            :param pedestrian_id: id of the actor to be updated.
            :param transform: new pedestrian transform (i.e., position and rotation).
            :param velocity: new pedestrian velocity. (direction * speed)
            :return: True if successfully updated. Otherwise, False.
        """
        ...
        last_transform = pedestrian.get_transform()

        distance = transform.distance(last_transform)
        if distance > 0.1:
    ...
    def tick(self):
        ...
        current_actors = set(
            [actor.id for actor in self.world.get_actors().filter('vehicle.*')] +
            [actor.id for actor in self.world.get_actors().filter('walker.pedestrian.*')])
